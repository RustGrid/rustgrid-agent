...
- Sandbox templates and kits are an early-access Docker feature; pin and stage-test template changes before rollout.
...
- Network access is required by the current Codex policy. Destination-level egress enforcement is not yet configured by this worker.
